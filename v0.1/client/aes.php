...
class AES
{
	...
	var $Rcon = array( array(0x00, 0x00, 0x00, 0x00),
	...
	function SubBytes($s, $Nb) {    // apply SBox to state S [§5.1.1]
	  ...
	  for ($r=0; $r<4; $r++) {
		for ($c=0; $c<$Nb; $c++) $s[$r][$c] = $this->Sbox[$s[$r][$c]];
	  }
	  return $s;
	}
	...
	function KeyExpansion($key) {  // generate Key Schedule from Cipher Key [§5.2]
	  ...
	  for ($i=$Nk; $i<($Nb*($Nr+1)); $i++) {
		...
		if ($i % $Nk == 0) {
		  ...
		  for ($t=0; $t<4; $t++) $temp[$t] ^= $this->Rcon[$i/$Nk][$t];
		} else if ($Nk > 6 && $i%$Nk == 4) {
	...
	function SubWord($w) {    // apply SBox to 4-byte word w
	  ...
	  for ($i=0; $i<4; $i++) $w[$i] = $this->Sbox[$w[$i]];
	  return $w;
	}
	...
	public function encrypt($plaintext, $password, $nBits) {
	  ...
	  for ($i=0; $i<4; $i++) $counterBlock[$i] = $this->urs($nonceSec, $i*8) & 0xff; 
	  ...
	  for ($b=0; $b<$blockCount; $b++) {
		...
		for ($c=0; $c<4; $c++) $counterBlock[15-$c-4] = $this->urs($b/100000000, $c*8);
	...
	public function decrypt($ciphertext, $password, $nBits) {
	  ...
	  for ($b=0; $b<$nBlocks; $b++) {
		// set counter (block #) in last 8 bytes of counter block (leaving nonce in 1st 8 bytes)
		for ($c=0; $c<4; $c++) $counterBlock[15-$c] = $this->urs($b, $c*8) & 0xff;
		for ($c=0; $c<4; $c++) $counterBlock[15-$c-4] = $this->urs(($b+1)/100000000-1, $c*8) & 0xff;
